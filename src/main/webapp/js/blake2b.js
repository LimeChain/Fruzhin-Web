export var Blake2b = {
    ...
    blake2bInit(outlen, key, salt, personal) {
        ...
        const ctx = {
            b: new Uint8Array(128), h: new Uint32Array(16), t: 0, c: 0, outlen
        };
    ...
    hash(message = "", secret = undefined, length = 64, salt = new Uint8Array(16), personal = new Uint8Array(16)) {
        if (secret?.length === 0) secret = undefined;
        if (typeof secret === "string") secret = new TextEncoder().encode(secret);
        const output = this.blake2bStart(message, secret, length, salt, personal);
